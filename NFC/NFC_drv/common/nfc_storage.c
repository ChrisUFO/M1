 ...
 */
/*============================================================================*/
static char *str_trim(char *s) { return str_ltrim(str_rtrim(s)); }
...
/*============================================================================*/
/**
 * @brief Parse HEX byte sequence from string (supports "AA BB CC" or "AABBCC"
 * format)
 ...
 * @return 0 on success, 1 if second digit missing, 2 if invalid char/hex, 3 if
 * buffer full
 */
/*============================================================================*/
 ...
 */
/*============================================================================*/
static void mark_unit_valid(uint8_t *valid_bits, uint32_t valid_bytes,
                            uint32_t idx) {
  if (!valid_bits)
    return;
  uint32_t byte_idx = idx >> 3;
  uint8_t bit_mask = (uint8_t)(1u << (idx & 7));
  if (byte_idx >= valid_bytes)
    return;
 ...
 *
 * Uses privateprofilestring.c for header fields (Filetype, Version, Device
 * type, UID, ATQA, SAK, ATS) This provides consistency with RFID parsing and
 ...
 */
/*============================================================================*/
static nfc_storage_result_t
nfc_storage_parse_header_ini(nfc_run_ctx_t *c, nfc_family_info_t *faminfo,
  ...
  if (parse_device_type((char *)data.buf, faminfo) != 0) {
  ...
  // data.v.hex.out_len); platformLog("[NFC Storage] UID tmp buffer: %s\r\n",
  ...
  if (faminfo->tech == M1NFC_TECH_A) {
    ...
    data.max_len = 2;
    data.type = VALUE_TYPE_HEX_ARRAY;
    if (GetPrivateProfileHex(&data, "ATQA", file_path) &&
        data.v.hex.out_len == 2) {
  ...
  if (faminfo->tech == M1NFC_TECH_A) {
    ...
    data.max_len = 1;
    data.type = VALUE_TYPE_HEX_ARRAY;
    if (GetPrivateProfileHex(&data, "SAK", file_path) &&
        data.v.hex.out_len == 1) {
  ...
  if (faminfo->tech == M1NFC_TECH_A) {
    ...
    data.type = VALUE_TYPE_HEX_ARRAY;
    if (GetPrivateProfileHex(&data, "ATS", file_path) &&
        data.v.hex.out_len > 0) {
 ...
 */
/*============================================================================*/
static void nfc_storage_store_unit(uint8_t *dump_buf, uint16_t unit_size,
                                   uint32_t unit_count, uint8_t *valid_bits,
                                   uint32_t valid_bits_bytes,
                                   uint32_t *max_seen_unit, uint32_t idx,
                                   const uint8_t *src_bytes, size_t src_len) {
  if (!dump_buf || unit_size == 0 || unit_count == 0)
    return;
  ...
  if (src_len < unit_size) {
    ...
    if (idx < unit_count) {
      ...
      if (max_seen_unit && idx > *max_seen_unit)
        *max_seen_unit = idx;
    }
    return;
  }
...
/*============================================================================*/
static nfc_storage_result_t
nfc_storage_parse_body(nfc_run_ctx_t *c, const nfc_family_info_t *faminfo,
                       uint8_t *dump_buf, uint32_t dump_buf_bytes,
                       uint8_t *valid_bits, uint32_t valid_bits_bytes) {
  ...
  while (1) {
    ...
    if (strncmp(line, "Block ", 6) == 0) {
      ...
      *colon = '\0';
      ...
      nfc_storage_store_unit(dump_buf, unit_size, unit_count, valid_bits,
                             valid_bits_bytes, &max_seen, (uint32_t)idx, bytes,
                             len);
      continue;
    }
  ...
  nfc_ctx_set_dump(unit_size, unit_count, 0, /* origin */
                   dump_buf, valid_bits, max_seen,
                   true); // dump set
 ...
 */
/*============================================================================*/
nfc_storage_result_t nfc_storage_load_file(const char *path, uint8_t *dump_buf,
...

{
  ...
  ret = nfc_storage_parse_body(c, &faminfo, dump_buf, dump_buf_bytes,
