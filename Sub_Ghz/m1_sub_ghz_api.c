...
#define SI4463_CTS_TIMEOUT                                                     \
  12000 // Number of SPI retries to read the CTS ready status
...
const uint8_t Radio_300_Configuration_Data_Array[] =
    RADIO_300_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_300 = RADIO_300_CONFIGURATION_DATA;

const uint8_t Radio_310_Configuration_Data_Array[] =
    RADIO_310_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_310 = RADIO_310_CONFIGURATION_DATA;

const uint8_t Radio_315_Configuration_Data_Array[] =
    RADIO_315_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_315 = RADIO_315_CONFIGURATION_DATA;

const uint8_t Radio_345_Configuration_Data_Array[] =
    RADIO_345_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_345 = RADIO_345_CONFIGURATION_DATA;

const uint8_t Radio_372_Configuration_Data_Array[] =
    RADIO_372_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_372 = RADIO_372_CONFIGURATION_DATA;

const uint8_t Radio_390_Configuration_Data_Array[] =
    RADIO_390_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_390 = RADIO_390_CONFIGURATION_DATA;

const uint8_t Radio_433_Configuration_Data_Array[] =
    RADIO_433_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_433 = RADIO_433_CONFIGURATION_DATA;

const uint8_t Radio_433_92_Configuration_Data_Array[] =
    RADIO_433_92_CONFIGURATION_DATA_ARRAY;
const tRadioConfiguration RadioConfiguration_433_92 =
    RADIO_433_92_CONFIGURATION_DATA;
...
const tRadioConfiguration RadioConfiguration_Patch =
    RADIO_PATCH_CONFIGURATION_DATA;

const tRadioConfiguration *RadioConfigList[SUB_GHZ_BAND_EOL] = {
    &RadioConfiguration_300, &RadioConfiguration_310,
    &RadioConfiguration_315, &RadioConfiguration_345,
    &RadioConfiguration_372, &RadioConfiguration_390,
    &RadioConfiguration_433, &RadioConfiguration_433_92,
...
void SI446x_Get_IntStatus(uint8_t PH_CLR_PEND, uint8_t MODEM_CLR_PEND,
                          uint8_t CHIP_CLR_PEND);
struct si446x_reply_GET_MODEM_STATUS_map *
SI446x_Get_ModemStatus(uint8_t MODEM_CLR_PEND);
...
void Radio_Start_Rx(uint8_t CHANNEL, uint8_t CONDITION, uint16_t RX_LEN,
...
/******************************************************************************/
/*
 * This function resets the radio after power up and waits for the CTS to get
 * ready
 *
 */
/******************************************************************************/
...
/******************************************************************************/
void SI446x_Reset(void) {
  // The SDN pin needs to be held high for at least 10us before driving low
  // again
...
/******************************************************************************/
/*
 * Get the Interrupt status/pending flags form the radio and clear flags if
 * requested. PH_CLR_PEND     Packet Handler pending flags clear. MODEM_CLR_PEND
 * Modem Status pending flags clear. CHIP_CLR_PEND   Chip State pending flags
 * clear.
 */
/******************************************************************************/
void SI446x_Get_IntStatus(uint8_t PH_CLR_PEND, uint8_t MODEM_CLR_PEND,
...
SI446x_Get_ModemStatus(uint8_t MODEM_CLR_PEND) {
  ...
  si446x_cmd.GET_MODEM_STATUS.AFC_FREQ_OFFSET =
      ((uint16_t)si446x_cmd_buffer[6] << 8) & 0xFF00;
  si446x_cmd.GET_MODEM_STATUS.AFC_FREQ_OFFSET |=
      (uint16_t)si446x_cmd_buffer[7] & 0x00FF;
...
/******************************************************************************/
/*
 * This function sends the PART_INFO command to the radio and receives the
 * answer
 */
/******************************************************************************/
struct si446x_reply_PART_INFO_map *SI446x_PartInfo(void) {
...
struct si446x_reply_REQUEST_DEVICE_STATE_map *SI446x_Request_DeviceState(void) {
  ...
  SI446x_Get_Resp(SI446X_CMD_REPLY_COUNT_REQUEST_DEVICE_STATE,
                  si446x_cmd_buffer);
  ...
  // *SI446x_Request_DeviceState(void)

/******************************************************************************/
/*
 * This function is used to load all properties and commands with a list of NULL
 * terminated commands. Commands structure in the array:
 * --------------------------------
...
uint8_t SI446x_ConfigInit(const uint8_t *pSetPropCmd) {
  ...
  while (*pSetPropCmd != 0x00) {
    ...
    if (HAL_GPIO_ReadPin(SI4463_nINT_GPIO_Port, SI4463_nINT_Pin) ==
        GPIO_PIN_RESET) {
      /* Get and clear all interrupts.  An error has occured... */
      SI446x_Get_IntStatus(0, 0, 0);
      /*
                  if (si446x_cmd.GET_INT_STATUS.CHIP_PEND &
...
*/
  ...
  // REMAIN	0	Remain in RX state (but do not re-arm to acquire another
  // packet). RX	8	RX state (briefly exit and re-enter RX state to
  // re-arm for acquisition of another packet). RXTIMEOUT_STATE - RXVALID_STATE
  // - RXINVALID_STATE
  // Radio_Start_Rx(channel, 0, RadioConfiguration.Radio_PacketLength, 0, 8, 8);
  Radio_Start_Rx(
      channel, 0, RADIO_XX_CONFIGURATION_DATA_RADIO_PACKET_LENGTH,
      SI446X_CMD_CHANGE_STATE_ARG_NEXT_STATE1_NEW_STATE_ENUM_NOCHANGE,
      SI446X_CMD_CHANGE_STATE_ARG_NEXT_STATE1_NEW_STATE_ENUM_RX,
      SI446X_CMD_CHANGE_STATE_ARG_NEXT_STATE1_NEW_STATE_ENUM_RX);
#else
  SI446x_Change_State(
      SI446X_CMD_CHANGE_STATE_ARG_NEXT_STATE1_NEW_STATE_ENUM_SPI_ACTIVE);
...
void SI446x_Start_Tx(uint8_t channel, uint8_t *pradio_tx_buffer,
  ...
  Radio_Start_Tx(tx_channel, START_TX_COMPLETE_STATE_READY,
                 length); // Enter the Ready state after completion of the
                          // packet transmission
} // void SI446x_Start_Tx(uint8_t channel, uint8_t *pradio_tx_buffer, uint8_t
  // length)
...
void SI446x_Calibrate_IR(void) {
  ...
  // note: looking at si446x_ircal.c in provided examples, CMD_IRCAL has 3
  // undocumented return values CAL_STATE, RSSI, DIR_CH
#ifdef SI446X_RADIO_IRCAL
  SI446x_Send_Cmd(SI446X_CMD_ARG_COUNT_IRCAL,
                  (uint8_t *)Radio_IRcal_Sequence_Coarse);
  SI446x_Send_Cmd(SI446X_CMD_ARG_COUNT_IRCAL,
                  (uint8_t *)Radio_IRcal_Sequence_Fine);
 ...
 * Reads the radio response back FIFO  RX/TX FIFO reset flags.
 */
/******************************************************************************/
void SI446x_FiFoInfo(uint8_t FIFO) {
...
/******************************************************************************/
/*
 * Selects the type of modulation. In TX mode, additionally selects the source
 * of the modulation. NEW_MOD_TYPE:  TX_DIRECT_MODE_TYPE
 * TX_DIRECT_MODE_GPIO	MOD_SOURCE	MOD_TYPE
 */
/******************************************************************************/
void SI446x_Change_ModType(uint8_t NEW_MOD_TYPE) {
...
void SI446x_Change_Radio_Setting(uint8_t mode, uint8_t pa_power) {
  ...
  case RADIO_SETTING_MODE_PAPOWER:
    ...
    si446x_cmd_buffer[3] =
        SI446X_GRP_ID_0x22_PA_PWR_LVL; // first property index in the group
...
void radio_init_rx_tx(S_M1_SubGHz_Band freq, uint8_t mod_type, bool do_reset) {
  ...
  while (SI446X_SUCCESS !=
         SI446x_ConfigInit(pradioconfig->Radio_ConfigurationArray)) {
    HAL_Delay(1); // 10
...
} // void radio_init_rx_tx(S_M1_SubGHz_Band freq, uint8_t mod_type, bool
  // do_reset)
...
void radio_patch_init(void) {
  ...
  if (SI446X_SUCCESS ==
    ...
    // SI446X_SUCCESS==SI446x_ConfigInit(pradioconfig->Radio_ConfigurationArray)
    // )
