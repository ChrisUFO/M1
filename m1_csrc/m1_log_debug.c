...
#define IS_BUFFER_VALID(pbuffer)                                               \
  ((pbuffer != NULL) && (pbuffer->pdata != NULL) && (pbuffer->len > 0))
...
static volatile uint16_t
    logdb_dma_tx_len; // This variable may be modified by an interrupt

/* Capture ring buffer for CLI 'log' command recall */
#define LOG_CAPTURE_BUF_SIZE 2048
static uint8_t log_capture_buf[LOG_CAPTURE_BUF_SIZE];
static S_M1_RingBuffer log_capture_rb;
...
/*
 * printf() will call _write() which will call __io_putchar() by default to send
 * data to UART Both _write() and __io_putchar() are defined as (weak) in
 * syscalls.c
 */
...
static void m1_logdb_dyn_vsprintf(const char *format, va_list pargs,
                                  char **pstring);
...
void m1_logdb_init(void) {
  ...
  */
  ...
  if (HAL_UARTEx_SetTxFifoThreshold(&huart_logdb, UART_TXFIFO_THRESHOLD_1_8) !=
      HAL_OK) {
    Error_Handler();
  }

  if (HAL_UARTEx_SetRxFifoThreshold(&huart_logdb, UART_RXFIFO_THRESHOLD_1_8) !=
      HAL_OK) {
    Error_Handler();
  }
  ...
  if (HAL_DMA_ConfigChannelAttributes(&hdma_rxlogdb, DMA_CHANNEL_NPRIV) !=
      HAL_OK) {
    Error_Handler();
  }
  ...
  HAL_NVIC_SetPriority(
      GPDMA1_Channel2_IRQn, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY,
      0); // configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY + 1, 5);
  ...
  hdma_logdb.Init.TransferAllocatedPort =
  ...
  if (HAL_DMA_ConfigChannelAttributes(&hdma_logdb, DMA_CHANNEL_NPRIV) !=
      HAL_OK) {
    Error_Handler();
  }
  ...
  /* GPDMA1 interrupt init */
  HAL_NVIC_SetPriority(GPDMA1_Channel1_IRQn,
                       configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY, 0); //, 5);
  HAL_NVIC_EnableIRQ(GPDMA1_Channel1_IRQn);

  // Clear tx buffer
  memset((void *)logdb_tx_buffer, 0x00, M1_LOGDB_TX_BUFFER_SIZE);

  m1_ringbuffer_init(&logdb_tx_rb, logdb_tx_buffer, M1_LOGDB_TX_BUFFER_SIZE,
                     sizeof(uint8_t));
  m1_ringbuffer_init(&log_capture_rb, log_capture_buf, LOG_CAPTURE_BUF_SIZE,
                     sizeof(uint8_t));

  logdb_dma_tx_len = 0;
  plogdb_tx_rb =
      &logdb_tx_rb; // Use this pointer for macro to avoid compile error

#ifdef M1_DEBUG_CLI_ENABLE
  /* Enable the UART global Interrupt */
  HAL_NVIC_SetPriority(LOG_DEBUG_UART_IRQn,
  ...
  if (m1_usbcdc_mode == CDC_MODE_VCP) {
    ...
    HAL_UART_Receive_DMA(&huart_logdb, (uint8_t *)logdb_rx_buffer,
...
static void m1_logdb_start_dma_tx(void) {
  if (!logdb_dma_tx_len) {
    ...
    //__HAL_DMA_ENABLE_IT(&hdma_logdb, (DMA_IT_TC | DMA_IT_DTE | DMA_IT_ULE |
    // DMA_IT_USE | DMA_IT_TO));

    // DMA_SetConfig()
    /* Configure the DMA channel data size */
    MODIFY_REG(hdma_logdb.Instance->CBR1, DMA_CBR1_BNDT,
               (logdb_dma_tx_len & DMA_CBR1_BNDT));
    /* Clear all interrupt flags */
    __HAL_DMA_CLEAR_FLAG(&hdma_logdb, DMA_FLAG_TC | DMA_FLAG_HT | DMA_FLAG_DTE |
                                          DMA_FLAG_ULE | DMA_FLAG_USE |
                                          DMA_FLAG_SUSP | DMA_FLAG_TO);
    /* Configure DMA channel source address */
    hdma_logdb.Instance->CSAR =
        (uint32_t)m1_ringbuffer_get_read_address(plogdb_tx_rb);
...
int __io_putchar(int ch) {
  ...
  while ((__HAL_UART_GET_FLAG(&huart_logdb, UART_FLAG_TXE) ? SET : RESET) ==
         RESET) {
  ...
  while ((__HAL_UART_GET_FLAG(&huart_logdb, UART_FLAG_TC) ? SET : RESET) ==
         RESET) {
    if ((HAL_GetTick() - tickstart) > UART_PUT_CHAR_TIMEOUT) {
      return EOF; // Timeout error
    }
...
int _write(int file, char *data, int len) {
  ...
  /* Also write to capture buffer for CLI 'log' command recall */
  m1_ringbuffer_write(&log_capture_rb, (uint8_t *)data, len);
...
void log_db_handler_task(void *param) {
  ...
  while (1) {
    ...
    if (ret == pdPASS) {
      if ((hUsbDeviceFS.pClassData != NULL) && (m1_USB_CDC_ready == 0)) {
...
} // void m1_logdb_printf(S_M1_LogDebugLevel_t level, const char* tag, const
  // char* format, ...)
...
static void m1_logdb_dyn_vsprintf(const char *format, va_list pargs,
  ...
  while (1) {
    ...
    *pstring = NULL; // Caller function may check NULL for error condition
    if (mem_size > 2 * M1_LOGDB_MESSAGE_SIZE) // Memory size exceeds the maximum
                                              // allowed size?
      break;                                  // Exit
...
} // static void m1_logdb_dyn_vsprintf(const char *format, va_list pargs, char
  // **pstring)

/*============================================================================*/
/*
 * Read recent log messages from the capture buffer without consuming them.
 * Returns the number of bytes written to 'out'.
 */
/*============================================================================*/
size_t m1_logdb_get_recent(char *out, size_t max_len) {
  if (out == NULL || max_len == 0)
    return 0;

  /* Read available data from the capture ring buffer */
  uint32_t available = ringbuffer_get_data_slots(&log_capture_rb);
  if (available == 0) {
    strncpy(out, "(No log messages captured)\r\n", max_len);
    out[max_len - 1] = '\0';
    return strlen(out);
  }

  /* Limit to output buffer size minus null terminator */
  uint16_t to_read =
      (uint16_t)((available < (max_len - 1)) ? available : (max_len - 1));

  /* Non-destructive read: peek at the data then restore the read pointer.
     The ring buffer read function advances the tail, so we save/restore it. */
  uint32_t saved_tail = log_capture_rb.tail;
  uint16_t actually_read =
      m1_ringbuffer_read(&log_capture_rb, (uint8_t *)out, to_read);
  log_capture_rb.tail = saved_tail; /* Restore — don't consume the data */
