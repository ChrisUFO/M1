...
#define SUBGHZ_TX_RAW_REPLAY_REPEAT_DEFAULT                                    \
  0 // 2 plus the first transmit before repeating
...
#define SUB_GHZ_433_92_NEW_PDTC                                                \
  0x6C // Attack and decay times of the OOK Peak Detector - Default: 0x28
...
// #define SUBGHZ_FCC_ISM_BAND_304_100				(float)304.10001
// // FZ
...
static const char *subghz_band_text[] = {"300.000", "310.000", "315.000",
                                         "345.000", "372.000", "390.000",
                                         "433.000", "433.920", "915.000"};

static const char *subghz_datfile_keywords[SUB_GHZ_DATAFILE_KEY_FORMAT_N] = {
    "Filetype:", "Version:", "Frequency:", "Modulation:", "Protocol:",
    "Bits:",     "Payload:", "BT:",        "IT:"};
...
static const float subghz_fcc_ism_bands_NA[SUBGHZ_ISM_BANDS_LIST_NA][2] = {
    {SUBGHZ_FCC_ISM_BAND_310_000,
     SUBGHZ_FCC_ISM_BAND_321_950}, // 304.10MHz - 321.95MHz
    /*{SUBGHZ_FCC_ISM_BAND_344_000, SUBGHZ_FCC_ISM_BAND_392_000},
       // 344.00MHz - 392.00MHz*/
    {SUBGHZ_FCC_ISM_BAND_433_050,
     SUBGHZ_FCC_ISM_BAND_434_790}, // 433.05MHz - 434.79MHz
    {SUBGHZ_FCC_ISM_BAND_915_000,
     SUBGHZ_FCC_ISM_BAND_928_000} // 915.00MHz - 928.00MHz
};
...
static const float subghz_fcc_ism_bands_ASIA[SUBGHZ_ISM_BANDS_LIST_ASIA][2] = {
    ...
    /*{SUBGHZ_FCC_ISM_BAND_344_000, SUBGHZ_FCC_ISM_BAND_392_000},
       // 344.00MHz - 392.00MHz*/
    /*{SUBGHZ_FCC_ISM_BAND_433_050, SUBGHZ_FCC_ISM_BAND_434_790}, 	//
       433.05MHz - 434.79MHz*/
    {SUBGHZ_FCC_ISM_BAND_915_000,
     SUBGHZ_FCC_ISM_BAND_928_000} // 915.00MHz - 928.00MHz
};
...
const S_M1_SUBGHZ_ISM_REGIONS_t
    subghz_regions_list[SUBGHZ_ISM_BAND_REGIONS_LIST] = {
...
S_M1_SubGHz_Scan_Config subghz_scan_config = {.band = SUB_GHZ_BAND_300,
...
static uint8_t sub_ghz_replay_start(bool record_mode, S_M1_SubGHz_Band band,
...
/*============================================================================*/
void menu_sub_ghz_init(void) {
  // First radio init with reset (do_reset is true) will load the radio patch
  // SI446X_PATCH_CMDS! Other calls to radio_init_rx_tx() will load new
...
void sub_ghz_init(void) {
  ...
  HAL_NVIC_SetPriority((IRQn_Type)(SI4463_nIRQ_EXTI_IRQn),
...
static void sub_ghz_set_opmode(uint8_t opmode, uint8_t band, uint8_t channel,
  ...
  case SUB_GHZ_OPMODE_TX:
    ...
    // Direct mode asynchronous mode, TX direct mode on GPIO2,  modulation is
    ...
    Radio_Start_Tx(
        channel, START_TX_COMPLETE_STATE_NOCHANGE,
        0); // Do not change state after completion of the packet transmission
...
} // static void sub_ghz_set_opmode(uint8_t opmode, uint8_t band, uint8_t
  // channel, uint8_t tx_power)
...
void sub_ghz_record(void) {
  ...
  // GUI init
  m1_uiView_functions_init(VIEW_MODE_SUBGHZ_RECORD_EOL,
                           view_subghz_record_table);
...
/*============================================================================*/
static void subghz_record_gui_init(void) {
  m1_uiView_functions_register(
      VIEW_MODE_SUBGHZ_RECORD, subghz_record_gui_create,
      subghz_record_gui_update, subghz_record_gui_destroy,
      subghz_record_gui_message);
} // static void subghz_record_gui_init(void)
...
static void subghz_record_gui_update(uint8_t param) {
  ...
  case SUBGHZ_RECORD_DISPLAY_PARAM_READY:
    ...
    u8g2_SetDrawColor(&m1_u8g2,
                      M1_DISP_DRAW_COLOR_BG); // set the color to White
    ...
    u8g2_SetDrawColor(&m1_u8g2, M1_DISP_DRAW_COLOR_TXT);
    u8g2_DrawBox(&m1_u8g2, 0, 52, 128,
                 12); // Draw an inverted bar at the bottom to display options
    u8g2_SetDrawColor(&m1_u8g2,
...
static int subghz_record_kp_handler(void) {
  ...
  if (ret == pdTRUE) {
    if (this_button_status.event[BUTTON_BACK_KP_ID] ==
        BUTTON_EVENT_CLICK) // Exit or Stop
    {
      if (subghz_uiview_gui_latest_param ==
          SUBGHZ_RECORD_DISPLAY_PARAM_ACTIVE) {
        // This case is handled the same way
        // with the case the [BUTTON_OK_KP_ID] is pressed
        // This code is copied from that case below. It can be placed in a
        // sub-function if needed.
        ...
        m1_sdm_task_deinit();
        sub_ghz_set_opmode(SUB_GHZ_OPMODE_ISOLATED, subghz_scan_config.band, 0,
                           0);
      ...
      else if (subghz_uiview_gui_latest_param ==
               SUBGHZ_RECORD_DISPLAY_PARAM_COMPLETE) {
        sub_ghz_raw_samples_deinit(true); // Discard samples
        m1_uiView_display_update(SUBGHZ_RECORD_DISPLAY_PARAM_READY);
      } // else if (
        // subghz_uiview_gui_latest_param==SUBGHZ_RECORD_DISPLAY_PARAM_COMPLETE
        // )
      else if (subghz_uiview_gui_latest_param ==
               SUBGHZ_RECORD_DISPLAY_PARAM_PLAY) {
        sub_ghz_raw_tx_stop();
        ...
        // subghz_uiview_gui_latest_param==SUBGHZ_RECORD_DISPLAY_PARAM_PLAY )
      else {
             ...
             BUTTON_EVENT_CLICK) // Start/Stop
    {
      if (subghz_uiview_gui_latest_param == SUBGHZ_RECORD_DISPLAY_PARAM_READY) {
        ...
        if (!ret) {
          ...
          xQueueReset(main_q_hdl); // Reset old samples in the queue, if any
          m1_ringbuffer_reset(
              &subghz_rx_rawdata_rb); // Reset sample ring buffer
          m1_led_fast_blink(LED_BLINK_ON_RGB, LED_FASTBLINK_PWM_M,
                            LED_FASTBLINK_ONTIME_M); // Turn on
          sub_ghz_tx_raw_deinit();
        ...
        // nfc_uiview_gui_latest_param==NFC_READ_DISPLAY_PARAM_READING_COMPLETE
        // )
      else if (subghz_uiview_gui_latest_param ==
               SUBGHZ_RECORD_DISPLAY_PARAM_ACTIVE) {
        sub_ghz_rx_pause(); // Stop receiving
        ...
        subghz_replay_ret_code =
            sub_ghz_replay_start(true, subghz_scan_config.band, 0, 255);
        ...
        // )
               ...
               SUBGHZ_RECORD_DISPLAY_PARAM_PLAY) {
        if (subghz_replay_ret_code !=
            SUB_GHZ_RAW_DATA_PARSER_IDLE) // Do nothing if it's replaying
          return 1;
        ...
        if (subghz_replay_ret_code != 1) {
          ...
          // Display error message at the bottom line if the function x_reinit()
          // failed.
          m1_uiView_display_update(SUBGHZ_RECORD_DISPLAY_PARAM_SYS_ERROR);
        } // else
      } // else if (
    ...
    else if (this_button_status.event[BUTTON_LEFT_KP_ID] ==
             BUTTON_EVENT_CLICK) // Left
    {
      if (subghz_uiview_gui_latest_param == SUBGHZ_RECORD_DISPLAY_PARAM_READY) {
        ...
        // sub_ghz_set_opmode(SUB_GHZ_OPMODE_TX, subghz_scan_config.band, 0,
        // 255);
        m1_uiView_display_update(SUBGHZ_RECORD_DISPLAY_PARAM_READY);
        // m1_ringbuffer_reset(&subghz_rx_rawdata_rb); // Reset rx buffer for
        // new frequency
      } // if (
      ...
      // )
    else if (this_button_status.event[BUTTON_RIGHT_KP_ID] ==
             BUTTON_EVENT_CLICK) // Right
    {
      if (subghz_uiview_gui_latest_param == SUBGHZ_RECORD_DISPLAY_PARAM_READY) {
...
static void subghz_replay_browse_gui_init(void) {
  m1_uiView_functions_register(
      VIEW_MODE_SUBGHZ_REPLAY_BROWSE, subghz_replay_browse_gui_create,
      subghz_replay_browse_gui_update, subghz_replay_browse_gui_destroy,
      subghz_replay_browse_gui_message);
} // static void subghz_replay_browse_gui_init(void)
...
static void subghz_replay_browse_gui_update(uint8_t param) {
  ...
  while (true) {
    ...
    m1_uiView_display_switch(VIEW_MODE_SUBGHZ_REPLAY_PLAY,
                             SUBGHZ_REPLAY_DISPLAY_PARAM_ACTIVE);
    menu_sub_ghz_init();
...
static void subghz_replay_play_gui_init(void) {
  m1_uiView_functions_register(
      VIEW_MODE_SUBGHZ_REPLAY_PLAY, subghz_replay_play_gui_create,
      subghz_replay_play_gui_update, subghz_replay_play_gui_destroy,
      subghz_replay_play_gui_message);
} // static void subghz_replay_play_gui_init(void)
             ...
             BUTTON_EVENT_CLICK) // Start/Stop
    {
      if (subghz_uiview_gui_latest_param == SUBGHZ_REPLAY_DISPLAY_PARAM_PLAY) {
        if (subghz_replay_ret_code !=
            SUB_GHZ_RAW_DATA_PARSER_IDLE) // Do nothing if it's replaying
          return 1;
        sub_ghz_set_opmode(SUB_GHZ_OPMODE_TX, subghz_replay_band,
                           subghz_replay_channel, 255);
        subghz_replay_ret_code = sub_ghz_raw_replay_init();
        if (subghz_replay_ret_code != 1) {
          double_buffer_ptr_id = 1;
...
static uint8_t sub_ghz_file_load(void) {
  ...
  do {
    ...
    while (uret) {
      key_len++;
      if (f_info->file_name[--uret] ==
          '.') // Find the dot starting from the last character
        break;
    } // while ( uret )
    ...
    for (subghz_replay_band = 0; subghz_replay_band < SUB_GHZ_BAND_EOL;
         subghz_replay_band++) {
      freq_min = subghz_band_steps[subghz_replay_band][0];
      freq_max = freq_min;
      freq_max +=
          0.25 *
          subghz_band_steps[subghz_replay_band][1]; // Channel spacing = 0.25MHz
    ...
    while (subghz_replay_freq > freq_min) {
      freq_min += 0.25; // Increase channel until the selected frequency matches
                        // closely with the desired frequency
      subghz_replay_channel++;
    } // while ( subghz_replay_freq > freq_min )

    token = strtok(NULL, "\r\n"); // Modulation
    m1_strtoupper(token);
    for (subghz_replay_mod = 0; subghz_replay_mod < SUBGHZ_MODULATION_LIST;
         subghz_replay_mod++) {
...
void sub_ghz_replay(void) {
  ...
  // GUI init
  m1_uiView_functions_init(VIEW_MODE_SUBGHZ_REPLAY_EOL,
                           view_subghz_replay_table);
  m1_uiView_display_switch(VIEW_MODE_SUBGHZ_REPLAY_BROWSE, 0);
...
void sub_ghz_frequency_reader(void) {
  ...
  while (1) // Main loop of this task
  {
    ...
    if (freq_step <= subghz_band_steps[active_band_id][1]) {
      ...
      sub_ghz_set_opmode(
          SUB_GHZ_OPMODE_RX, active_band_id, 0,
          0); // Process time: ~27.57ms (with reset) - ~4.5ms (optimized)
      if (subghz_band_steps[active_band_id][1] >
          1)           // There're other channels to retry
    ...
    if (rssi_max >=
        (rssi_avg + SIGNAL_TO_NOISE_RATIO)) // SNR matches the condition?
    {
    ...
    if (++freq_step > subghz_band_steps[active_band_id]
                                       [1]) // All channels have been completed?
    {
      ...
      while (true) {
        ...
        if (subghz_band_steps[active_band_id][1] !=
            0) // Change to this band if it is not disabled
          break;
      } // while (true)
    } // if ( ++freq_step > subghz_band_steps[active_band_id][1] )
...
void sub_ghz_regional_information(void) {
  ...
  S_M1_Main_Q_t q_item;
  BaseType_t ret;
  uint8_t i, row, freq_l[10], freq_h[10], freq_range[32];
...
static void sub_ghz_rx_init(void) {
  ...
  /* SMS = 1000:Combined reset + trigger mode - Rising edge of the selected
   * trigger input (tim_trgi) reinitializes the counter, generates an update of
   * the registers and starts the counter. SMS = 0100: Reset mode - Rising edge
  ...
  if (HAL_TIM_IC_ConfigChannel(&timerhdl_subghz_rx, &tim_ic_init,
                               SUBGHZ_RX_TIMER_RX_CHANNEL) != HAL_OK) {
    //_Error_Handler(__FILE__, __LINE__);
    Error_Handler();
  }
...
/*============================================================================*/
static void sub_ghz_rx_deinit(void) {
  if (timerhdl_subghz_rx.State ==
      HAL_TIM_STATE_READY) // Make sure that the timer has been initialized!
  {
...
static uint8_t sub_ghz_ring_buffers_init(void) {
  ...
  while (subghz_front_buffer) {
    subghz_ring_read_buffer = malloc(SUBGHZ_RAW_DATA_SAMPLES_TO_RW *
                                     2); // Each sample has a 2-byte value
    ...
    if (!subghz_sdcard_write_buffer)
      break;
    m1_ringbuffer_init(&subghz_rx_rawdata_rb, (uint8_t *)subghz_front_buffer,
                       subghz_front_buffer_size, sizeof(uint16_t));
...
static void sub_ghz_tx_raw_init(void) {
  ...
  if (HAL_TIM_ConfigClockSource(&timerhdl_subghz_tx, &sClockSourceConfig) !=
      HAL_OK) {
    Error_Handler();
  }
  ...
  if (HAL_TIMEx_MasterConfigSynchronization(&timerhdl_subghz_tx,
  ...
  if (HAL_TIM_PWM_ConfigChannel(&timerhdl_subghz_tx, &sConfigOC,
                                SUBGHZ_TX_TIMER_TX_CHANNEL) != HAL_OK) {
    Error_Handler();
  }
  ...
  if (HAL_TIMEx_ConfigBreakDeadTime(&timerhdl_subghz_tx,
  ...
  hdma_subghz_tx.Init.TransferAllocatedPort =
      DMA_SRC_ALLOCATED_PORT0 | DMA_DEST_ALLOCATED_PORT0;
  ...
  if (HAL_DMA_ConfigChannelAttributes(&hdma_subghz_tx, DMA_CHANNEL_NPRIV) !=
      HAL_OK) {
    Error_Handler();
  }
  ...
  __HAL_DMA_CLEAR_FLAG(&hdma_subghz_tx,
                       DMA_FLAG_TC | DMA_FLAG_HT | DMA_FLAG_DTE | DMA_FLAG_ULE |
                           DMA_FLAG_USE | DMA_FLAG_SUSP | DMA_FLAG_TO);
  // IRQ priority should be lower than that of the Timer
  HAL_NVIC_SetPriority(GPDMA1_Channel0_IRQn,
                       configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY, 5);
  ...
  HAL_NVIC_SetPriority(SUBGHZ_TX_TIMER_IRQn,
                       configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY, 0);
...
static void sub_ghz_tx_raw_deinit(void) {
  ...
  if (hdma_subghz_tx.Instance != NULL) {
    ...
    __HAL_DMA_DISABLE_IT(&hdma_subghz_tx, (DMA_IT_TC | DMA_IT_DTE | DMA_IT_ULE |
...
/*============================================================================*/
/**
 * @brief This function allocates memory for data buffers, reads data file
 * header and checks its format
 * @param  None
 * @retval error code
 */
/*============================================================================*/
static uint8_t sub_ghz_raw_samples_init(void) {
  ...
  do {
    ...
    sdcard_dat_buffer +=
        M1_SDM_MIN_BUFFER_SIZE / 2; // Start at the middle of the buffer
    sdcard_dat_read_size =
        M1_SDM_MIN_BUFFER_SIZE /
        4; // Limit the reading size from SD card to avoid data error
    ...
    M1_LOG_D(M1_LOGDB_TAG, "sub_ghz_raw_samples_init %d\r\n",
             subghz_back_buffer_size);
    raw_samples_buffer_size =
        (subghz_back_buffer_size < subghz_front_buffer_size)
            ? subghz_back_buffer_size
            : subghz_front_buffer_size;
    ...
    memcpy(psdcard_dat_buffer, sdcard_dat_buffer,
           sdcard_dat_read_size + 1); // Duplicate this file header
    ...
    sdcard_dat_buffer_end_pos =
        (uint32_t)sdcard_dat_buffer + sdcard_dat_read_size;
...
static void sub_ghz_raw_samples_deinit(bool discard_samples) {
  ...
  if (sdcard_dat_buffer) {
    sdcard_dat_buffer -=
        M1_SDM_MIN_BUFFER_SIZE /
        2; // Restore the original allocated address of the buffer
...
static uint8_t sub_ghz_parse_raw_data(uint8_t buffer_ptr_id) {
  ...
  while (true) {
    ...
    if (token == NULL) {
      ...
      do {
        if (token == NULL) // Possibly last few characters of this buffer
        {
          ...
          sdcard_dat_buffer_end_pos -=
              (uint32_t)sdcard_buffer_run_ptr; // Remaining characters
          if (sdcard_dat_buffer_end_pos) // Are there remaining characters?
          {
            // If the write buffer address is not aligned, the data read from
            // SDcard may be corrupted randomly when writing. So if there're
            // remaining characters, let copy the last few characters to before
            // the beginning of the aligned buffer
            sdcard_dat_buffer -=
                sdcard_dat_buffer_end_pos; // Move backward a few positions
            memcpy(sdcard_dat_buffer, sdcard_buffer_run_ptr,
                   sdcard_dat_buffer_end_pos);
          ...
          if (sdcard_dat_file_size == 0)                // End of file?
          {
            ...
            sdcard_buffer_run_ptr =
                sdcard_dat_buffer; // Update moving buffer pointer
            sdcard_buffer_run_ptr -=
                sdcard_dat_buffer_end_pos; // Update address for remaining
                                           // characters, if any
            sdcard_dat_buffer_end_pos =
                (uint32_t)sdcard_dat_buffer; // Update moving buffer pointer
            sdcard_dat_buffer_end_pos +=
                sdcard_dat_read_size; // Update buffer end index for this data
                                      // block
            sdcard_dat_buffer[sdcard_dat_read_size] =
                '\0'; // Add end of string to the buffer
            token = (char *)strstr((char *)sdcard_buffer_run_ptr,
    ...
    *token = '\0'; // Add end of string to replace the " " at the end
    ...
    if (*token == '+' || *token == '-')
      *token =
          '0'; // Replace the +/- sign before the number with number 0, if any
    number = strtol(token, &endptr, 10);
    if (number == 0 && endptr == token) // Not a valid number?
    {
      if (strstr(token, SUB_GHZ_DATAFILE_DATA_KEYWORD) ==
          NULL) // Not the main key word?
      {
        if (strlen(token) != 0) // In case the data file has more than one CR/LF
                                // at the end of file
        {
...
static void sub_ghz_transmit_raw(uint32_t source, uint32_t dest, uint32_t len,
  ...
  // len &= ~(uint16_t)1; // Make length an even number. DMA doesn't work with
  // odd length for unknown reason!
  ...
  HAL_StatusTypeDef ret =
      TIM_DMA_Start_IT(timerhdl_subghz_tx.hdma[TIM_DMA_ID_UPDATE], source, dest,
                       subghz_decenc_ctl.ntx_raw_len);
  if (ret != HAL_OK)
    return;
  ...
  // Do it again to reload the DMA source data[1] to the ARR register, and
...
} // static void sub_ghz_transmit_raw(uint32_t source, uint32_t dest, uint32_t
  // len, uint8_t repeat)
...
static void sub_ghz_transmit_raw_restart(uint32_t source, uint32_t len) {
  ...
  MODIFY_REG(hdma_subghz_tx.Instance->CBR1, DMA_CBR1_BNDT,
             (subghz_decenc_ctl.ntx_raw_len & DMA_CBR1_BNDT));
  /* Clear all interrupt flags */
  __HAL_DMA_CLEAR_FLAG(&hdma_subghz_tx,
                       DMA_FLAG_TC | DMA_FLAG_HT | DMA_FLAG_DTE | DMA_FLAG_ULE |
                           DMA_FLAG_USE | DMA_FLAG_SUSP | DMA_FLAG_TO);
...
static uint8_t sub_ghz_rx_raw_save(bool header_init, bool last_data) {
  ...
  if (last_data) {
    ...
    if (n_samples_to_rw >
        SUBGHZ_RAW_DATA_SAMPLES_TO_RW) // This should never happen!
      n_samples_to_rw = SUBGHZ_RAW_DATA_SAMPLES_TO_RW;
  } // if ( last_data )
  m1_ringbuffer_read(&subghz_rx_rawdata_rb, subghz_ring_read_buffer,
                     n_samples_to_rw);
  // sub_ghz_rx_start();
...
static uint8_t sub_ghz_raw_replay_init(void) {
  ...
  while (!ret_code) {
    ...
    sub_ghz_transmit_raw_restart((uint32_t)double_buffer_ptr[0],
                                 raw_samples_count);
    if (ret_code ==
        SUB_GHZ_RAW_DATA_PARSER_READY) // There're more samples to read?
...
static uint8_t sub_ghz_replay_start(bool record_mode, S_M1_SubGHz_Band band,
  ...
  if (record_mode) {
    sub_ghz_rx_deinit();
    m1_sdm_task_stop(); // Stop sampling raw data and flush data to SD card and
                        // then close file
    ret_code = sub_ghz_raw_samples_init();
  } // if ( record_mode )
  while (!ret_code) {
    ...
    if (raw_samples_count ==
        0) // No samples found in Record mode or Replay mode?
    {
    ...
    sub_ghz_transmit_raw((uint32_t)double_buffer_ptr[0],
  ...
  return ret_code;
} // static uint8_t sub_ghz_replay_start(bool record_mode, S_M1_SubGHz_Band
  // band, uint8_t channel, uint8_t power)
...
static uint8_t sub_ghz_replay_continue(uint8_t ret_code_in) {
  ...
  case SUB_GHZ_RAW_DATA_PARSER_COMPLETE:
    sub_ghz_transmit_raw_restart(
        (uint32_t)double_buffer_ptr[double_buffer_ptr_id], raw_samples_count);
    double_buffer_ptr_id ^= 1; // Update raw samples buffer
...
static uint8_t sub_ghz_fcc_ism_band_check(uint8_t band, uint8_t channel) {
  ...
  for (i = 0;
       ...
       subghz_regions_list[m1_device_stat.config.ism_band_region].bands_list;
       i++) {
    if ((freq >= subghz_regions_list[m1_device_stat.config.ism_band_region]
                     .this_region[i][0]) &&
        (freq <= subghz_regions_list[m1_device_stat.config.ism_band_region]
                     .this_region[i][1])) {
      ret = 0;
      break;
    }
  } // for (i=0;
    // i<subghz_regions_list[m1_device_stat.config.ism_band_region].bands_list;
    // i++)
...
/*============================================================================*/
/**
 * @brief Rotate the ring buffer, move [tail] or read pointer to the first
...
static void sub_ghz_buffer_rotate(S_M1_RingBuffer *prb_handle) {
  ...
  while (next != first) {
    ...
    // copy [first] to [next]
    memcpy(&prb_handle->pdata[next * data_size],
           &prb_handle->pdata[first * data_size], data_size);
    // copy [next] to [first]
