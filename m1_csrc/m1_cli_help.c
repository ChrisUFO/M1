...
void cmd_m1_mtest_help_basic_system(void) {
  ...
  M1_LOG_N(M1_LOGDB_TAG,
           "Syntax: mtest frequency(Hz) duration(millisecond)\r\n");
  vTaskDelay(1); // Give the log task some time to do its job
...
void cmd_m1_mtest_help_led(void) {
  ...
  M1_LOG_N(M1_LOGDB_TAG, "Syntax: mtest 20 r_g_b(1-2-4 or combined) "
                         "rgb_pwm(0-255) on_off_time(0-65535)\r\n");
  vTaskDelay(1); // Give the log task some time to do its job
...
void cmd_m1_mtest_help_infrared(void) {
  ...
  M1_LOG_N(M1_LOGDB_TAG,
           "Syntax: mtest 40 protocol address command repeat\r\n");
  vTaskDelay(1); // Give the log task some time to do its job
...
void cmd_m1_mtest_help_subghz(void) {
  ...
  M1_LOG_N(M1_LOGDB_TAG,
           "Syntax: mtest 60 frequency(304, 310, 315, 345, 390, 433, 915)\r\n");
  vTaskDelay(1); // Give the log task some time to do its job
  ...
  M1_LOG_N(M1_LOGDB_TAG,
           "Syntax: mtest 64 network(0:315, 1:433, 2:915, 3:None)\r\n");
  vTaskDelay(1); // Give the log task some time to do its job
