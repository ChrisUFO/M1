...
#define BUTTON_EVENT_RESTORED                                                  \
  0x02 // A button active event restores to idle (released) state

#define SYSTEM_PERIODIC_TASK_DELAY                                             \
  BUTTON_DEBOUNCE_MS /                                                         \
      2 // ms - Period for the task to read buttons' press-release status
...
S_GPIO_IO_t m1_buttons_io[NUM_BUTTONS_MAX] = {
...
S_M1_Buttons_Status m1_buttons_status = {
    .event = {BUTTON_EVENT_IDLE, BUTTON_EVENT_IDLE, BUTTON_EVENT_IDLE,
              BUTTON_EVENT_IDLE, BUTTON_EVENT_IDLE, BUTTON_EVENT_IDLE},
    .timestamp = 0x00};
...
void startup_error_screen_display(const char *l1, const char *l2);
...
void system_periodic_task(void *param) {
  ...
  while (TRUE) {
    ...
    for (i = 0; i < NUM_BUTTONS_MAX; i++) {
      this_button_level = HAL_GPIO_ReadPin(m1_buttons_io[i].gpio_port,
                                           m1_buttons_io[i].gpio_pin);
      ...
      case BUTTON_PRESS_ATTEMPT:
        temp = current_tick; // get current timer tick
        temp -= buttons_ctl[i]
                    .click_counter; // calculate the time past for de-bounce
        if (temp >= BUTTON_DEBOUNCE_MS) {
          if (this_button_level == buttons_ctl[i].active_level) // still pressed
          {
            ...
            case BUTTON_DBC_CLK1:
              ...
              temp -=
                  buttons_ctl[i]
                      .dbc_counter; // calculate the duration between two clicks
              if (temp <= BUTTON_DBC_MIDDLE) {
                buttons_ctl[i].dbc_status =
                    BUTTON_DBC_CLK2; // valid double click received
                buttons_ctl[i].dbc_counter =
                    current_tick; // store current timer tick
              } else {
                buttons_ctl[i].dbc_status =
                    BUTTON_DBC_IDLE; // invalid double click received, reset
              }
              break;

            case BUTTON_DBC_CLK2:
              buttons_ctl[i].dbc_status =
                  BUTTON_DBC_IDLE; // new single click received, reset
              break;
      ...
      case BUTTON_RELEASE_ATTEMPT:
        temp = current_tick; // get current timer tick
        temp -= buttons_ctl[i]
                    .click_counter; // calculate the time past for debounce
        if (temp >= BUTTON_DEBOUNCE_MS) {
          if (this_button_level !=
              buttons_ctl[i].active_level) // still released
          {
            ...
            case BUTTON_DBC_IDLE:
              ...
              temp -=
                  buttons_ctl[i]
                      .dbc_counter; // calculate the duration of the first click
            ...
            case BUTTON_DBC_CLK2:
              buttons_ctl[i].dbc_status =
                  BUTTON_DBC_IDLE; // double click released, reset
              buttons_ctl[i].event =
                  BUTTON_EVENT_IDLE; // reset if not reset by caller function
              event_change = BUTTON_EVENT_RESTORED;
              break;
    ...
    if (event_change) {
      for (i = 0; i < NUM_BUTTONS_MAX; i++) {
        m1_buttons_status.event[i] =
            buttons_ctl[i].event;                 // Update latest status
        buttons_ctl[i].event = BUTTON_EVENT_IDLE; // Reset
      }
      m1_device_stat.active_timestamp =
          current_tick; // Update latest time stamp
    } // if ( event_change )

    if (m1_device_stat.op_mode != M1_OPERATION_MODE_FIRMWARE_UPDATE) {
      if (event_change &
          BUTTON_EVENT_ACTIVE) // Notify task only when there's an active event
      {
    ...
    m1_wdt_send_report(M1_REPORT_ID_BUTTONS_HANDLER_TASK,
                       SYSTEM_PERIODIC_TASK_DELAY);
  } // while (TRUE)
...
static void send_button_evt_to_queue(void) {
  ...
  if (ret) // Is there a button event?
  {
    ...
    default:
      ret = xQueueReceive(button_events_q_hdl, &this_button_status,
                          0); // Remove this button event
      break;
    } // switch(m1_device_stat.op_mode)
  } // if ( ret )
} // static void send_button_evt_to_queue(void)
  ...
  if (SystemPowerStatus.fault == 0) {
    ...
    if (old_stat != new_stat) // New status?
    {
      ...
      case 0: // Not charging
        ...
        } else if (running_id == LED_BATTERY_CHARGED_ON_FN_ID ||
                   running_id == LED_BATTERY_FULL_ON_FN_ID) {
      ...
      case 2:
        ...
        } else if (running_id == LED_INDICATOR_OFF_FN_ID ||
                   running_id != LED_BATTERY_CHARGED_ON_FN_ID) {
...
void startup_device_init(void) {
  ...
  // This bit is set by hardware and cleared only by a POR or by setting the
  // CSSF bit. 0: system has not been in Standby mode. 1: system has been in
  // Standby mode.
  ...
  {
    // HAL_PWR_EnableWakeUpPin(PWR_WAKEUP_PIN4_LOW); // Enable WKUP4 pin,
    // falling edge HAL_RTCEx_DeactivateWakeUpTimer(&hrtc);
    // HAL_PWR_EnterSTANDBYMode(); // Enter Standby mode
    ;
  } // if ( !stat )

  k = (sizeof(S_M1_BK_REGS_t) + 3) / 4;
  bu_reg_read = RTC_BKP_DR0; // Backup address, address offset: 0x100 + 0x04 *
                             // x, (x = 0 to 31)
...
void startup_config_write(uint8_t config_byte, uint8_t config_val) {
  ...
  bu_reg_write = RTC_BKP_DR0; // Backup address, address offset: 0x100 + 0x04 *
                              // x, (x = 0 to 31)
  switch (config_byte) {
...
static void startup_bu_registers_init(void) {
  ...
  m1_device_stat.bu_regs.magic_number =
      SYS_CONFIG_MAGIC_NUMBER;          // Assign magic number
...
void startup_info_screen_display(const char *scr_text) {
  ...
  u8g2_DrawXBMP(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X, M1_POWERUP_LOGO_TOP_POS_Y,
                M1_POWERUP_LOGO_WIDTH, M1_POWERUP_LOGO_HEIGHT, m1_logo_40x32);

  sprintf(fw_ver, "Version %d.%d.%d", m1_device_stat.config.fw_version_major,
          m1_device_stat.config.fw_version_minor,
          m1_device_stat.config.fw_version_build);
  len = strlen(fw_ver);
  u8g2_SetFont(&m1_u8g2, M1_POWERUP_LOGO_FONT);
  u8g2_DrawStr(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X + M1_POWERUP_LOGO_WIDTH + 3,
               M1_POWERUP_LOGO_TOP_POS_Y + 15, "MONSTATEK M1");
  u8g2_SetFont(&m1_u8g2, M1_DISP_MAIN_MENU_FONT_N);
  u8g2_DrawStr(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X + M1_POWERUP_LOGO_WIDTH + 3,
               M1_POWERUP_LOGO_TOP_POS_Y + 25, fw_ver);
  u8g2_DrawStr(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X + M1_POWERUP_LOGO_WIDTH + 3,
               M1_POWERUP_LOGO_TOP_POS_Y + 35, FW_VERSION_FORK_TAG);
...
void startup_error_screen_display(const char *l1, const char *l2) {
  ...
  u8g2_SetFont(&m1_u8g2, M1_POWERUP_LOGO_FONT);
  u8g2_DrawStr(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X + M1_POWERUP_LOGO_WIDTH + 3,
               M1_POWERUP_LOGO_TOP_POS_Y + 15, "UPDATE FAILED");
  u8g2_SetFont(&m1_u8g2, M1_DISP_MAIN_MENU_FONT_N);
  u8g2_DrawStr(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X + M1_POWERUP_LOGO_WIDTH + 3,
               M1_POWERUP_LOGO_TOP_POS_Y + 25, l1);
  u8g2_DrawStr(&m1_u8g2, M1_POWERUP_LOGO_LEFT_POS_X + M1_POWERUP_LOGO_WIDTH + 3,
               M1_POWERUP_LOGO_TOP_POS_Y + 35, l2);

  u8g2_SetFont(&m1_u8g2, M1_DISP_MAIN_MENU_FONT_B);
  const char *ack_str = "Press OK to boot";
  len = strlen(ack_str);
  x0 = (M1_LCD_DISPLAY_WIDTH - len * M1_GUI_FONT_WIDTH) / 2;
  if (x0 >= M1_GUI_FONT_WIDTH)
    x0 -= M1_GUI_FONT_WIDTH;
  u8g2_DrawStr(&m1_u8g2, x0, 62, ack_str);
...
/*============================================================================*/
/*
 * This function does a non-blocking delay. It replaces the default HAL_Delay()
 * function
 */
/*============================================================================*/
void HAL_Delay(uint32_t Delay) {
