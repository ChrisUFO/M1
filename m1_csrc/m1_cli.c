...
BaseType_t cmd_m1_mtest(char *pconsole, size_t xWriteBufferLen,
                        const char *pcCommandString, uint8_t num_of_params);
void cmd_m1_mtest_basic_system(char *pconsole, char *input_params[],
                               uint8_t n_params, uint8_t cmd_type);
void cmd_m1_mtest_sdcard(char *pconsole, char *input_params[], uint8_t n_params,
                         uint8_t cmd_type);
void cmd_m1_mtest_led(char *pconsole, char *input_params[], uint8_t n_params,
                      uint8_t cmd_type);
void cmd_m1_mtest_lcd(char *pconsole, char *input_params[], uint8_t n_params,
                      uint8_t cmd_type);
void cmd_m1_mtest_infrared(char *pconsole, char *input_params[],
                           uint8_t n_params, uint8_t cmd_type);
void cmd_m1_mtest_power(char *pconsole, char *input_params[], uint8_t n_params,
                        uint8_t cmd_type);
void cmd_m1_mtest_subghz(char *pconsole, char *input_params[], uint8_t n_params,
                         uint8_t cmd_type);
void cmd_m1_mtest_esp32(char *pconsole, char *input_params[], uint8_t n_params,
                        uint8_t cmd_type);
void cmd_m1_mtest_gpio(char *pconsole, char *input_params[], uint8_t n_params,
                       uint8_t cmd_type);
void cmd_m1_mtest_nfc(char *pconsole, char *input_params[], uint8_t n_params,
                      uint8_t cmd_type);
...
/*============================================================================*/
/*
 * This command runs a specific test based on the command type given in the
 * parameters
 */
/*============================================================================*/
BaseType_t cmd_m1_mtest(char *pconsole, size_t xWriteBufferLen,
  ...
  (void)xWriteBufferLen; /* Unused: stub for future work. May need removal
                            later. */
  ...
  n_params =
      (num_of_params <= INPUT_PARAMS_MAX) ? num_of_params : INPUT_PARAMS_MAX;

  // Copy command string to a mutable buffer for parameter null-termination
  strncpy(pcCommandStringCopy, pcCommandString,
          sizeof(pcCommandStringCopy) - 1);
  ...
  for (i = 0; i < n_params; i++)
    input_params[i][input_params_len[i]] =
        0x00; // Add NULL to the end of string
...
} // BaseType_t cmd_m1_mtest(char *pconsole, size_t xWriteBufferLen, const char
  // *pcCommandString, uint8_t num_of_params)
...
BaseType_t cmd_dfu(char *pcWriteBuffer, size_t xWriteBufferLen,
  ...
  snprintf(pcWriteBuffer, xWriteBufferLen, "Rebooting to USB DFU mode...\r\n");
  firmware_update_usb_dfu_mode_cli();
  return pdFALSE;
} // BaseType_t cmd_dfu(char *pcWriteBuffer, size_t xWriteBufferLen, const char
  // *pcCommandString, uint8_t num_of_params)
  ...
  case 9:
    ...
    if (HAL_FLASH_Unlock() == HAL_OK) {
      if (HAL_FLASH_Program(FLASH_TYPEPROGRAM_QUADWORD, FW_CONFiG_ADDRESS,
        ...
        strcpy(pconsole, "Error programming flash!\r\n");
      }
      HAL_FLASH_Lock();
    } else {
...
void cmd_m1_mtest_sdcard(char *pconsole, char *input_params[], uint8_t n_params,
  ...
  case 10:
    ...
    ret =
        m1_fb_dyn_strcat(buffer, 2, "", SDCARD_CLI_DRIVE_PATH, input_params[1]);
    if (!ret)
      break;
  ...
  case 17:
    M1_LOG_N(M1_LOGDB_TAG,
             "CLI mtest: SD card - delete a file or directory\r\n");
    if (n_params < 2) {
      strcpy(pconsole, "Error: missing parameter(s)!\r\n");
      break;
    }
  ...
  case 19:
    M1_LOG_N(M1_LOGDB_TAG,
             "CLI mtest: SD card - list all files and folders\r\n");
    if (n_params < 2) {
      strcpy(pconsole, "Error: missing parameter(s)!\r\n");
      break;
    }
...
} // void cmd_m1_mtest_sdcard(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
} // void cmd_m1_mtest_led(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
void cmd_m1_mtest_lcd(char *pconsole, char *input_params[], uint8_t n_params,
  ...
  case 35:
    M1_LOG_N(M1_LOGDB_TAG,
             "CLI mtest: LCD - init retry when display fails\r\n");
...
} // void cmd_m1_mtest_lcd(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
void cmd_m1_mtest_infrared(char *pconsole, char *input_params[],
  ...
  case 40:
    ...
    while (1) {
      ...
      // subfunc_handler_task. This task is the sub-task of
      // subfunc_handler_task. The notification is given in the form of an item
      // in the main queue. So let read the main queue.
      ret = pdFALSE;
      force_quit = 0;
      q_item.q_evt_type = 0;
      ...
      if (ret == pdTRUE || force_quit) {
        if (q_item.q_evt_type == Q_EVENT_IRRED_TX ||
            force_quit) // Transmit completed?
        {
          if (infrared_transmit(0) ==
              IR_TX_ACTIVE) // There's still repeated packet to transmit?
            continue;       // Let repeat
...
} // void cmd_m1_mtest_infrared(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
} // void cmd_m1_mtest_power(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
void cmd_m1_mtest_subghz(char *pconsole, char *input_params[], uint8_t n_params,
  ...
  case 60:
    ...
    if (freq == SUB_GHZ_BAND_EOL) {
      M1_LOG_N(M1_LOGDB_TAG,
               "Warning: frequency unknown, switching to default config!\r\n");
  ...
  case 69:
    M1_LOG_N(M1_LOGDB_TAG,
             "CLI mtest: Sub-GHz - get device operating state\r\n");
    pdevstate = SI446x_Request_DeviceState();
    M1_LOG_N(M1_LOGDB_TAG, "State: %d Channel: %d\r\n", pdevstate->CURR_STATE,
             pdevstate->CURRENT_CHANNEL);
...
} // void cmd_m1_mtest_subghz(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
} // void cmd_m1_mtest_esp32(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
} // void cmd_m1_mtest_gpio(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
...
void cmd_m1_mtest_nfc(char *pconsole, char *input_params[], uint8_t n_params,
  ...
  M1_LOG_N(M1_LOGDB_TAG,
           "CLI mtest: NFC - CLI TEST input_params[%s] cmd_type[%d] "
           "n_params[%d]\r\n",
...
} // void cmd_m1_mtest_nfc(char *pconsole, char *input_params[], uint8_t
  // n_params, uint8_t cmd_type)
