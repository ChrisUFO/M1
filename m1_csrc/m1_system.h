...
// #define BUTTON_DBC_MIDDLE   		500 // maximum interval between two
// clicks for double click recognition
#define BUTTON_DBC_MIDDLE 300
#define BUTTON_DBC_CLICK                                                       \
  500 // maximum click duration for double click recognition
...
#define BUTTON_PRESS_STATE                                                     \
  BUTTON_LOGIC_LOW // logic level of the button when pressed
#define BUTTON_RELEASE_STATE                                                   \
  BUTTON_LOGIC_HIGH // logic level of the button when released (idle)

#define BUTTON_REPEATED_PRESS_ENABLE // Enable repeated press event to replace
...
// Boot operation status codes moved to m1_fw_update_bl.h

typedef enum {
  M1_OPERATION_MODE_POWER_UP =
      0x01, // Power is up and the M1 is still in power off mode (shutdown mode)
  M1_OPERATION_MODE_DISPLAY_ON, // M1 is turned on. Default welcome screen is
                                // displayed
...
typedef struct {
  S_M1_Key_Event event; // single click, double click or long press event; reset
                        // after read by caller function
