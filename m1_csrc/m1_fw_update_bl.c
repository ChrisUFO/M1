...
#include "main.h"
#include "stm32h5xx.h" // For raw register access
...
uint8_t bl_crc_check(uint32_t image_size) {
  ...
  result = HAL_CRC_Calculate(
      &crchdl, (uint32_t *)(FW_START_ADDRESS + M1_FLASH_BANK_SIZE), image_size);
  ...
  M1_LOG_E(M1_LOGDB_TAG,
           "crc32: 0x%lX, cal_crc32: 0x%lX, 32-bit image size: %lu\r\n",
           (unsigned long)crc32, (unsigned long)result,
...
uint32_t bl_get_crc_chunk(uint32_t *data_scr, uint32_t len, bool crc_init,
  ...
  if (crc_init) {
    ...
    crchdl.Init.DefaultInitValueUse = DEFAULT_INIT_VALUE_ENABLE;
    crchdl.Init.InputDataInversionMode =
        CRC_INPUTDATA_INVERSION_NONE; // CRC_INPUTDATA_INVERSION_WORD;
    crchdl.Init.OutputDataInversionMode =
        CRC_OUTPUTDATA_INVERSION_DISABLE; // CRC_OUTPUTDATA_INVERSION_ENABLE;
    crchdl.InputDataFormat = CRC_INPUTDATA_FORMAT_WORDS;
    if (HAL_CRC_Init(&crchdl) != HAL_OK) {
      return BL_CODE_CHK_ERROR;
    }
...
static uint8_t bl_flash_start(uint32_t image_size) {
  ...
  /*
   An STM32H5 crashing when accessing the FLASH_BANK_SIZE register is typically
   caused by accessing unwritten memory, triggering an Error Correction Code
   (ECC) fault. Accessing this unwritten memory causes a Double Error Detection
   (DED), which raises a Non-Maskable Interrupt (NMI). Solution: Read the Flash
   ECC Data Register (FLASH_ECCDR): Inside the NMI handler, check the
   FLASH_ECCDR to identify the nature of the ECC error. uint32_t eccdr_value =
   FLASH->ECCDR; // Read ECCDR to clear the NMI NMI_Handler
   */
...
static uint8_t bl_flash_binary(uint8_t *payload, size_t size) {
  ...
  if (!init_done) {
    ...
    flash_add +=
        M1_FLASH_BANK_SIZE; // It should always write to Bank 2 destination
    write_acc = 0;
    init_done = true;
  } // if ( !init_done )
  ...
  write_acc -=
      FW_IMAGE_CRC_SIZE; // exclude the CRC at the end of the image file
...
uint8_t bl_flash_app(FIL *hfile) {
  ...
  while (write_size) {
    ...
    count =
        m1_fb_read_from_file(hfile, (char *)fw_payload, FW_IMAGE_CHUNK_SIZE);
...
void fw_gui_progress_update(size_t remainder) {
  ...
  if (image_size < remainder) {
    ...
    u8g2_SetDrawColor(&m1_u8g2,
                      M1_DISP_DRAW_COLOR_BG); // set to background color
    // Draw solid box to clear existing content
    u8g2_DrawBox(&m1_u8g2, 4,
                 INFO_BOX_Y_POS_ROW_1 - M1_SUB_MENU_FONT_HEIGHT + 1, 120,
                 M1_SUB_MENU_FONT_HEIGHT);
    u8g2_SetDrawColor(&m1_u8g2, M1_DISP_DRAW_COLOR_TXT); // return to text color
    u8g2_DrawXBMP(&m1_u8g2, FW_UPDATE_PROGRESS_SLIDE_STRIP_COL,
                  FW_UPDATE_PROGRESS_SLIDE_STRIP_ROW, 126, 14,
                  fw_update_slide_strip_126x14); // Progress slide strip
  ...
  if (percent_cnt) {
    ...
    if (percent_cnt) {
      ...
      u8g2_DrawBox(&m1_u8g2, 4,
                   ...
                   M1_SUB_MENU_FONT_HEIGHT);
      u8g2_SetDrawColor(&m1_u8g2,
                        M1_DISP_DRAW_COLOR_TXT); // return to text color
      u8g2_DrawStr(&m1_u8g2, 4, INFO_BOX_Y_POS_ROW_1,
                   (char *)percent_txt); // Write new content
      u8g2_DrawXBMP(&m1_u8g2, progress_slider_x_post,
                    FW_UPDATE_PROGRESS_SLIDE_STRIP_ROW + 3, 4, 8,
                    fw_update_slider_5x8); // Progress slider
      progress_slider_x_post +=
          FW_UPDATE_SLIDER_WIDTH - FW_UPDATE_SLIDER_OVERLAP;
...
/*============================================================================*/
/**
 * @brief  Boot-time integrity check executed strictly before RAM
 * initialization.
 * @param  None
 * @retval None
 */
...
void boot_recovery_check(void) {
  uint32_t active_bank_start_addr = FW_START_ADDRESS;

  // Read the current SWAP_BANK state from FLASH Option Bytes register directly
  if (FLASH->OPTSR_CUR & FLASH_OPTSR_SWAP_BANK) {
    // Swap is ENABLED -> Bank 2 is mapped to 0x08000000
    // Our physical execution is happening from Bank 2 memory mapped to Bank 1
    // area. Wait, STM32H5 bank swap actually maps Bank 2 to 0x08000000. So the
    // read addresses 0x08000000 and 0x080FFC00 always point to the ACTIVE bank.
  }

  // We execute this from the ACTIVE bank (which is mapped to 0x08000000)
  // Read the fw_image_size from the active bank's config wrapper
  S_M1_FW_CONFIG_t *fw_cfg = (S_M1_FW_CONFIG_t *)(FW_CONFIG_RESERVED_ADDRESS);

  // Basic sanity checks on magic numbers and size
  if (fw_cfg->magic_number_1 != FW_CONFIG_MAGIC_NUMBER_1 ||
      fw_cfg->magic_number_2 != FW_CONFIG_MAGIC_NUMBER_2 ||
      fw_cfg->fw_image_size == 0 ||
      fw_cfg->fw_image_size > M1_FLASH_BANK_SIZE) {
    // Invalid config structure! We should treat this as a corruption.
    goto recovery_fallback;
  }

  // Enable CRC clock manually (since SystemInit is very early)
  __HAL_RCC_CRC_CLK_ENABLE();

  // Configure CRC peripheral for STM32 HAL default CRC32
  // Polynomial: 0x4C11DB7, Init: 0xFFFFFFFF
  CRC->CR = CRC_CR_RESET; // Reset CRC calculation unit
  // Wait for reset to complete (few cycles)
  __NOP();
  __NOP();

  // The CRC calculation requires 32-bit words
  uint32_t num_words = fw_cfg->fw_image_size / 4;
  uint32_t *data_ptr = (uint32_t *)active_bank_start_addr;
  ...
  uint32_t stored_crc = *(uint32_t *)(FW_CRC_ADDRESS);
...
recovery_fallback:
  ...
  // We must swap back to the other bank.

  // To prevent an infinite swap loop if BOTH banks are corrupted:
  // We will use a Backup Register to track the boot failure.
