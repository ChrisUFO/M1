...
#define FLASH_SYSTEM_MEMORY_ADDRESS 0x0BF90000
  ...
  do {
    for (i = 0; i < n_items; i++) {
      if (i == sel_item) {
        ...
        u8g2_DrawBox(
            &m1_u8g2, 0, menu_text_y - THIS_LCD_MENU_TEXT_ROW_SPACE + 2,
            M1_LCD_SUB_MENU_TEXT_FRAME_W, THIS_LCD_MENU_TEXT_ROW_SPACE);
        ...
        u8g2_SetFont(&m1_u8g2,
                     M1_DISP_SUB_MENU_FONT_N); // return to default font
      } else {
        u8g2_DrawStr(&m1_u8g2, 4, menu_text_y, phmenu->submenu[i]->title);
      }
    ...
    case 0: // Image file
      if (f_info && f_info->file_is_selected) {
        if (strlen(f_info->file_name) > GUI_DISP_LINE_LEN_MAX) {
          strncpy((char *)prn_name, f_info->file_name,
                  GUI_DISP_LINE_LEN_MAX - 2);
      ...
      case M1_FW_UPDATE_READY:
        ...
        sprintf((char *)prn_name, "New ver. %d.%d.%d.%d",
                (uint8_t)(fw_version_new >> 24),
                (uint8_t)(fw_version_new >> 16), (uint8_t)(fw_version_new >> 8),
                (uint8_t)fw_version_new);
        m1_info_box_display_draw(INFO_BOX_ROW_2, prn_name);
        sprintf((char *)prn_name, "Old ver. %d.%d.%d.%d",
    ...
    case 1: // Firmware update
      ...
      case M1_FW_UPDATE_SUCCESS:
        ...
        fw_update_status =
            M1_FW_UPDATE_NOT_READY;       // Reset after process complete
...
} // void firmware_update_gui_update(const S_M1_Menu_t *phmenu, uint8_t
  // sel_item)
...
void firmware_update_get_image_file(void) {
  ...
  do {
    ...
    u8g2_DrawBox(&m1_u8g2, M1_LCD_DISPLAY_WIDTH - 28,
                 THIS_LCD_MENU_TEXT_FIRST_ROW_Y + THIS_LCD_MENU_TEXT_ROW_SPACE +
                     2,
                 24, THIS_LCD_MENU_TEXT_ROW_SPACE);
    u8g2_SetDrawColor(&m1_u8g2, M1_DISP_DRAW_COLOR_TXT); // return to text color
    u8g2_DrawXBMP(&m1_u8g2, M1_LCD_DISPLAY_WIDTH - 24, 16, 18, 32,
                  hourglass_18x32); // Draw icon
    m1_u8g2_nextpage();             // Update display RAM

    uret =
        m1_fb_dyn_strcat(pfullpath, 2, "", f_info->dir_name, f_info->file_name);
    ...
    if (!uret) {
      ...
      if (image_size % 4 != 0) {
        M1_LOG_E(M1_LOGDB_TAG,
                 "Error: File size %lu not aligned to 4 bytes\r\n", image_size);
        uret = M1_FW_IMAGE_SIZE_INVALID;
        break;
      }
      ...
      sum = image_size;
      while (sum) {
        count = m1_fb_read_from_file(&hfile_fw, (char *)fw_payload,
                                     FW_IMAGE_CHUNK_SIZE);
        ...
        if (count < FW_IMAGE_CHUNK_SIZE) // possible last chunk?
        {
          ...
          if (count)
            crc32ret = bl_get_crc_chunk((uint32_t *)fw_payload,
                                        count / FW_IMAGE_CRC_SIZE, false,
                                        true); // complete
          else // last chunk is the appended CRC data
          {
            // Call the function to complete the transaction.
            // Do not take the return value in this case. Input data can be
            // anything.
          ...
          if (!sum) // end of file?
          {
            ...
            crc32ret =
                bl_get_crc_chunk((uint32_t *)fw_payload,
                                 count / FW_IMAGE_CRC_SIZE, false, false);
        } // else
      } // while ( sum )
      ...
      fret = f_lseek(
          &hfile_fw,
          FW_START_ADDRESS ^
              FW_CONFIG_ADDRESS); // Move file pointer to the config data
      if (fret != FR_OK) {
        uret = M1_FW_IMAGE_FILE_ACCESS_ERROR;
        break;
      } // if ( fret != FR_OK )
      count = m1_fb_read_from_file(&hfile_fw, (char *)&fwconfig,
                                   sizeof(S_M1_FW_CONFIG_t));
      ...
      (void)
          fwver_old; /* Unused: stub for future work. May need removal later. */
...
void firmware_update_JumpTo_BL(void) {
  ...
  (void)
      Bl_Signature; /* Unused: stub for future work. May need removal later. */

  // The first value stored in the vector table is the reset value of the stack
  // pointer
  if ((*(uint32_t *)FLASH_BL_ADDRESS & 0x2FFE0000) == 0x20000000) {
    ...
    /*
                    __asm volatile(
                                    " mov r0, #0			\n"
                                    " msr msp, r0			\n" //
       Reset Main Stack Pointer " msr psp, r0			\n" // Reset
       Process Stack Pointer " msr control, r0		\n" // Reset Control
       register
                    );
    */
    __set_CONTROL(0x00);
